...
public class WMXFileComparison extends TestBase {
	...
	String filePath1="C:\\QAPRODFiles\\FAILED"+"\\"+DateUtil.getDateString(new Date(), "yyyyMMdd");
	...
	String testTableName = "QA_COMPARE_DATA_1";
	String maskingTableName="cmx_client";
	String  interfaceID="XYZ";
	boolean saveRecords=false,saveFailedRecord=true,maskFlag;
	int NoOfRecordsToBeSaved=5, NoOfFailedRecordsToBeSaved=5 ;
	ResultSet rs1;
	PreparedStatement pstmt;
	int count=0,result,cnt=0;
	...
	public static int qaOrphanFileCount=0,maskOrphanFileCount;
		...
		try{
			...
			else
			{
				...
				else
				{
					...
					if(saveFailedRecord && cnt<NoOfFailedRecordsToBeSaved)					
	...
	public void generateReport() throws IOException 
	{
		...
		sb.append("<tr><th>TRANSMISSION_TYPE</h2></th>"+
				"<th>RECEIVERID</h2></th>"+
				"<th>PASS</th>"+
				"<th>FAIL</th>"+
				"<th>READY</th>"+
				...
				"<th>NEW</th>"+
				"<th>NO_MASK</th></tr>");
		String resultCount=" select  QCD_RECEIVERID_PR as RECEIVER_ID, QCD_TRANSMISSION_TYPE_PR as TRANSMISSION_TYPE , sum(decode(qcd_status,'PASS',1,0))as PASS, sum(decode(qcd_status,'FAIL',1,0))as FAIL, sum(decode(qcd_status,'READY',1,0))as READY, sum(decode(qcd_status,'XML',1,0))as XML, sum(decode(qcd_status,'NEW',1,0)) as NEW,sum(decode(qcd_status,'NO MASK',1,0)) as NO_MASK  from "+testTableName+" group by QCD_TRANSMISSION_TYPE_PR, QCD_RECEIVERID_PR  order by  QCD_RECEIVERID_PR";
		try
		{
		rs=dbt.executeSelectAndFetchResultset(resultCount);
		while(rs.next())
		{
		sb1.append("<tr><td>"+rs.getString(1)+"</td><td>"+rs.getString(2)+"</td><td>"+rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5)+"</td><td>"+rs.getString(6)+"</td><td>"+rs.getString(7)+"</td><td>"+rs.getString(8)+"</td></tr>");	
		}
		...
		String header_Report="<html><head><style>"
				...
				+"}"
				+"</style></head><body>"
				+"<h1>SC IT QA Automation Team</h1><hr /><h2>Test Execution Report: WMX File Comparision</h2> <hr />"
				+"<table>"
				+"<tr><th>Comparision Start Time: </th><td>"+executionStartTime+"</td></tr>"
				+"<tr><th>Comparision End Time:</th><td>"+executionEndTime+"</td></tr>"
				+ "<tr><th>Total Number of Files:</th><td>"+dbRow+"</td></tr>"
				+"<tr><th>Matching File Count:</th><td>"+matchingFileCount+"</td></tr>"
				+"<tr><th>Non Matching File Count:</th><td>"+nonmatchingFileCount+"</td></tr>"
				+"<tr><th>Production Orphan File Count :</th><td>"+prodOrphanFileCount+"</td></tr>"
				+"<tr><th>QA orphan File Count:</th><td>"+qaOrphanFileCount+"</td></tr>"
				+"<tr><th>No Mask File Count:</th><td>"+maskOrphanFileCount+"</td></tr>"
				+"<tr></tr></table><hr />"
				+"<table Class='alt'>"
				+sb.toString()
				+sb1.toString()
	...
	public void setComparisionData(String prod_file, String qa_file, String format,String transType,String receivertype,String id) throws Exception {
		...
		qa=new File(qaFile);
		String updateMaskedStatus=" update "+testTableName+" set qcd_status='NO MASK' where QCD_TRANSMISSION_TYPE_PR=? and QCD_RECEIVERID_PR=?";
	...
	
	{
		...
		String prodFilePath=filePath1+"\\"+dateTimestamp+"\\PROD"+"\\"+receiverId+"\\"+transcationType;
		String qaFilePath=filePath1+"\\"+dateTimestamp+"\\QA"+"\\"+receiverId+"\\"+transcationType;
		createDirectory(prodFilePath);
		createDirectory(qaFilePath);
